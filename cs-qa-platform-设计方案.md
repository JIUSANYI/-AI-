...
| 文档状态 | 已确认，实施中 |
| 关联文档 | 《需求分析文档》《技术栈文档》《UI 设计文档》《接口文档》 |
...
- `backend/` 是当前开发范围，由后端流程负责业务逻辑、API、数据库、Redis、外部服务适配和后端部署。
- `frontend/` 作为独立运行单元保留，负责页面和交互；当前阶段不修改其页面、样式、Node 依赖或业务实现。
- 前端与后端通过版本化的 REST/JSON 接口协作，接口契约以《接口文档》为准。
- Docker Compose 中前端和后端仍可同机运行，但分别构建、分别发布；后端发布不得重新构建或替换前端镜像。
...
| M5 前端 | 四页面 + API 客户端 + 401 刷新 | 完整 UI |
