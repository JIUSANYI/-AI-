...
async function request<T>(path: string, init: RequestInit = {}, retry = true, attempt = 0): Promise<T> {
  ...
  catch (error) { if (attempt < 2 && (init.method || "GET").toUpperCase() === "GET") return request<T>(path, init, retry, attempt + 1); throw error; }
  const payload = await response.json().catch(() => ({} as ApiError));
  if (response.status === 401 && retry && !isRefresh && path !== "/auth/logout") {
    const refreshed = await refreshAccessToken();
    if (refreshed) return request<T>(path, init, false);
  }
  if (response.status >= 500 && attempt < 2 && (init.method || "GET").toUpperCase() === "GET") return request<T>(path, init, retry, attempt + 1);
  if (!response.ok) {
