...
- Redis 原子限流：同一用户每小时最多 5 次、同一客户端 IP 每小时最多 20 次；限流依赖不可用时返回 503
...
前端访问令牌放在 `AuthContext`/模块级内存变量中，严禁写入 `localStorage`。刷新页面时通过 httpOnly Cookie 静默调用 refresh 恢复会话。API 封装集中在 `src/lib/api.ts`：自动附加令牌，401 只刷新并重放一次；4xx 不自动重试，幂等 GET 的 5xx 最多重试 3 次，POST 不自动重放，网络失败显示离线提示。
