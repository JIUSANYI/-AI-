...
本文件是编码 Agent 的实施约定。需求、架构、技术栈和 UI 设计文档是项目上下文的权威来源：
...
当前阶段由本开发 Agent 负责 `backend/` 和 `frontend/` 的工程实现、联调、测试与部署配置维护。UI 视觉方向、页面布局、交互细节和视觉验收由另一个模型负责；实现前必须遵守《UI 设计文档》，不得擅自改变已确认的视觉基线。两方通过《接口文档》和本仓库提交协作，接口或页面契约变化必须同步文档。
